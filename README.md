...
```mkdir ~/DuplicateMessageBot && cd ~/DuplicateMessageBot && touch .env && chmod 600 .env```, 
...
## SOCKS5 прокси
Если для Телеграм бота нужен прокси (например, на российских хостингах), то в **.py скрипте** есть строки
с **apihelper.proxy**, которые нужно раскомментировать. **Порт** нужно заменить на тот, на котором работает 
ваш **SOCKS5 клиент**, который, в свою очередь, уже отправляет трафик на ваш **прокси-сервер**